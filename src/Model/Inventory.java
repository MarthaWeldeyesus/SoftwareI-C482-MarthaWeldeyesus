...
public class Inventory {
   /**
     * The part ID for a part.
     */
    private static int partId = 0;

    /**
     * The product ID for a product. 
     */
     ...
     * @return A list of parts found, null if not found.
     */
    public static ObservableList<Part> lookupPart(String partName) {
        ObservableList<Part> partsFound = FXCollections.observableArrayList();

        allParts.stream().filter((part) -> (part.getName().equals(partName))).forEachOrdered((part) -> {
            partsFound.add(part);
        });
    ...
    public static ObservableList<Product> lookupProduct(String productName) {
        ...
        allProducts.stream().filter((product) -> (product.getName().equals(productName))).             forEachOrdered((product) -> {
